...
const TOM_SITUACAO: Record<habitacao.SituacaoContrato, TomStatus> = {
  EM_ELABORACAO: 'neutro',
  VIGENTE: 'institucional',
  SUSPENSO: 'atencao',
  RENEGOCIADO: 'institucional',
  QUITADO: 'sucesso',
  RESCINDIDO: 'perigo',
  TRANSFERIDO: 'neutro',
};
    ...
    <>
      <CabecalhoTela
        trilha={[
          ...
          { rotulo: 'Contratos e carnês', href: '/contratos' },
          { rotulo: contrato.protocolo },
        ]}
        titulo={contrato.protocolo}
        subtitulo={
          <span className="tabular">
            ...
            · {contrato.familia.codigo} ·{' '}
            <Link href={`/acompanhamento/${contrato.unidade.id}`} className="hover:underline">
      ...
      <CorpoTela className="max-w-5xl">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <EtiquetaStatus
            rotulo={habitacao.rotuloSituacaoContrato(contrato.situacao)}
            tom={TOM_SITUACAO[contrato.situacao]}
          />
        ...
        {contrato.motivoSituacao && (
          <div className="mt-4">
            <Aviso tom="warning">{contrato.motivoSituacao}</Aviso>
          </div>
        )}
        ...
        <section className="mt-6 rounded-lg border border-borda bg-surface p-5">
          ...
          {contrato.inadimplencia.fase !== 'EM_DIA' && (
            <div className="mt-4">
              <Aviso tom={contrato.inadimplencia.podePropoRescisao ? 'danger' : 'warning'}>
                <div className="flex flex-wrap items-center gap-2 font-bold">
                  <EtiquetaStatus
                    rotulo={habitacao.rotuloFaseInadimplencia(contrato.inadimplencia.fase)}
                    tom={TOM_FASE[contrato.inadimplencia.fase]}
                  />
                  <span>{contrato.inadimplencia.parcelasVencidas} parcela(s) vencida(s)</span>
                </div>
